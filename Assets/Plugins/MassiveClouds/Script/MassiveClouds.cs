...
namespace Mewlist
{
    ...
    public partial class MassiveClouds : MonoBehaviour
    {
        ...
        void SetExposureKeywords(Material mat)
        {
            if (ambientMode == AmbientMode.Auto)
            {
                mat.EnableKeyword("_HDRP_AMBIENT");
                if (fogColorMode == ColorMode.Ambient)
                    mat.EnableKeyword("_HDRP_FOG_AMBIENT");
                else
                    mat.DisableKeyword("_HDRP_FOG_AMBIENT");
                if (cloudColorMode == ColorMode.Ambient)
                    mat.EnableKeyword("_HDRP_CLOUD_AMBIENT");
                else
                    mat.DisableKeyword("_HDRP_CLOUD_AMBIENT");
                if (shadowColorMode == ColorMode.Ambient)
                    mat.EnableKeyword("_HDRP_SHADOW_AMBIENT");
                else
                    mat.DisableKeyword("_HDRP_SHADOW_AMBIENT");
            }
            else
            {
                mat.DisableKeyword("_HDRP_AMBIENT");
                mat.DisableKeyword("_HDRP_FOG_AMBIENT");
                mat.DisableKeyword("_HDRP_CLOUD_AMBIENT");
                mat.DisableKeyword("_HDRP_SHADOW_AMBIENT");
            }
