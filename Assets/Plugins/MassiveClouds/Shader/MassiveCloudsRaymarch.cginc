    ...
    int    iteration)
{
    ...
    float3 base = lerp(saturate(1 - _Lighting) * baseColor, screnCol, _Dissolve);
...
void VolumetricShadow(inout float4 col, half4 cloudLayer, half4 screeLayer, Ray ray, ScreenSpace ss, HorizontalRegion region)
{
...
//    screnCol = saturate(screnCol);
    ...
    // Ramp
    #if _RAMP_ON
    col.rgb = Ramp(lerp(col.rgb, screnCol.rgb, _RampStrength), _Lighting * lightFactor);
    #endif
